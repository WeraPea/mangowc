...
void gesture_touch_up(TouchGroup *tg, TouchPoint *t) {
	...
	uint32_t swipe =
		gesture_calculate_swipe(t->gesture_start_x, t->gesture_start_y,
	...
	if (wl_list_length(&tg->touch_points) == 1) {
		uint32_t edge = gesture_calculate_edge(
			tg->m, t->gesture_start_x, t->gesture_start_y, t->gesture_end_x,
			t->gesture_end_y);
		uint32_t distance = gesture_calculate_distance(
			tg->m, t->gesture_start_x, t->gesture_start_y, t->gesture_end_x,
			t->gesture_end_y, swipe);
