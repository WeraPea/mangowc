...
void parse_option(Config *config, char *key, char *value) {
	if (strcmp(key, "keymode") == 0) {
		...
		int32_t parsed = sscanf(
			...
			",%255[^,],%255s",
			raw_name, raw_mfact, raw_nmaster, raw_layout, raw_rr, raw_scale,
			raw_x, raw_y, raw_width, raw_height, raw_refresh, raw_overscan_top,
			raw_overscan_bottom, raw_overscan_left, raw_overscan_right);
