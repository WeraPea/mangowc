...
>>>>>>> 396ac189 (fix: correct touch motion surface-local coordinate calculation)
} TouchPoint;
...
double zoom_center_x = 0.0f;
double zoom_center_y = 0.0f;
double zoom_move_grab_x = 0.0f;
...
Client *find_closest_tiled_client(Client *c) {
	...
	wl_list_for_each(tc, &clients, link) {
		...
		if (logical_cursor_x >= tc->geom.x &&
			logical_cursor_x < tc->geom.x + tc->geom.width &&
			logical_cursor_y >= tc->geom.y &&
			logical_cursor_y < tc->geom.y + tc->geom.height) {
			return tc;
		}
